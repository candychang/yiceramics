	...
	function($rootScope, $timeout, $q, ngImageGalleryOpts){
		return {
			...
			template : 	'<div class="ng-image-gallery img-move-dir-{{imgMoveDirection}}" ng-class="{inline:inline}">'+
							...
							'<div ng-if="thumbnails && !inline" class="ng-image-gallery-thumbnails container">'+
							    '<div class="row">' +
							        '<div class="col-xs-12 col-sm-6 col-md-3 thumb" ng-repeat="image in images track by $index" ng-click="methods.open($index);" ng-style="{ \'background-image\': \'url(\' + (image.thumbUrl || image.url) + \')\' }" ng-attr-title="{{image.title || undefined}}">'+
							         //   '<img alt="{{image.title}}" class="thumb" ng-src="{{image.thumbUrl || image.url}}"/>'+
							        '</div>'+
							    '</div>'+
